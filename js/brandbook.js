(function () {
  const viewer = document.querySelector('[data-brandbook-viewer]');
  if (!viewer) return;
  ...
  function pagePath(page) {
    return `assets/images/brandbook/aya-muse-brandbook-${String(page).padStart(2, '0')}.jpg`;
  }
  ...
  function updateStack() {
    ...
    stack.innerHTML = pages.map((page, index) => `
      <button class="brandbook-stack__page brandbook-stack__page--${index + 1}${page === zoomedPage ? ' is-zoomed' : ''}" type="button" data-page="${page}" aria-label="Preview brandbook page ${page}">
        <img src="${pagePath(page)}" alt="Aya Muse Brandbook page ${page}">
  ...
  function buildThumbs() {
    ...
    thumbs.innerHTML = Array.from({ length: totalPages }, (_, index) => {
      const page = index + 1;
      return `
        <button type="button" data-page="${page}" aria-label="Show brandbook pages from ${page}">
